...
- **Integrasi keamanan terpusat** — autentikasi & otorisasi melalui **SSO/LDAP/OAuth2** perusahaan, role-based access control (RBAC) per rig/region sesuai struktur organisasi, serta audit trail untuk setiap akses/perubahan data — menggantikan model "buka file, semua orang lihat semua data" saat ini yang tidak sesuai untuk data operasional yang sensitif.
